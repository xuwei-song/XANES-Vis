...
def find_first_local_minimum(data, start_index):
    for i in range(start_index + 1, len(data) - 1):
        if data[i] < data[i-1] and data[i] <= data[i+1]:
            return i
    return None  # 如果没有找到局部最小值
...
def extract_xanes_features(energy, intensity):
    ...
    # Find pit (C) - look for the first local minimum after the white line
    pit_index = find_first_local_minimum(intensity_norm, wl_index)
    if pit_index is not None:
        pit_intensity = intensity_norm[pit_index]
        pit_energy = energy[pit_index]
    else:
        print("No local minimum found after the white line")
        pit_intensity = pit_energy = np.nan
